...
AFRAME.registerShader('test', {
  ...
  fragmentShader: `#define GLSLSANDBOX
#ifdef GLSLSANDBOX
#ifdef GL_ES
...
uniform sampler2D tex0; //;10=tex16.png
uniform sampler2D tex1; //;30=tex09.jpg
...
float noise( in vec3 x )
{
	...
	vec2 rgA = textureLod( iChannel0, (uv+0.5+vec2(0.0,0.0))/256.0, 0.0 ).yx;
    vec2 rgB = textureLod( iChannel0, (uv+0.5+vec2(1.0,0.0))/256.0, 0.0 ).yx;
    vec2 rgC = textureLod( iChannel0, (uv+0.5+vec2(0.0,1.0))/256.0, 0.0 ).yx;
    vec2 rgD = textureLod( iChannel0, (uv+0.5+vec2(1.0,1.0))/256.0, 0.0 ).yx;
...
vec4 mapTerrain( in vec3 pos )
{
	...
	float spe = 0.04;
	float localtime = 5.0 + (iGlobalTime-10.0)*0.5;

	q.xyz += 2.0*noise( 2.0*q )*vec3(1.0,3.0,1.0);

	float f;
	q *= vec3(1.0,2.0,1.0);
	q += dir*localtime*8.0*spe; f  = 0.50000*noise( q ); q = q*2.02;
	q += dir*localtime*4.0*spe; f += 0.25000*noise( q ); q = q*2.03;
	q += dir*localtime*2.0*spe; f += 0.12500*noise( q ); q = q*2.01;
    q += dir*localtime*1.0*spe; f += 0.06250*noise( q );
...
vec3 path( float localtime )
{
	return vec3( 2.5*cos(0.03*localtime), 1.5, 2.5*sin(0.03*localtime) );
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	// camera
	float localtime = 125.0-iGlobalTime+10.0;
	vec3 ro = path( localtime+0.0 );
	vec3 ta = vec3(0.0,-0.5,0.0);
	float roll = 0.15*cos(0.07*localtime);
